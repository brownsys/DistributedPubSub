...
public class TestPubSub {
  ...
  private static final class TestPubSubCallback extends
      Callback<PubSubProtos.StringMessage> {
  ...
  public void testBPubSub() throws InterruptedException, IOException {
    ...
    // Publish a message
    System.out
        .println("Publisher publishing " + "hello" + " on topic " + topic);
    client.publish(topic,
        PubSubProtos.StringMessage.newBuilder().setMessage("hello").build());
    cb.awaitMessage("hello");

    // Publish a message
    System.out.println("Publisher publishing " + "hello" + " on topic "
        + "badtest");
    client.publish("badtest", PubSubProtos.StringMessage.newBuilder()
        .setMessage("hello").build());
    cb.awaitNoMessage("hello");

    // Publish a message
    System.out.println("Publisher publishing " + "hello2" + " on topic "
        + topic);
    client.publish(topic,
        PubSubProtos.StringMessage.newBuilder().setMessage("hello2").build());
